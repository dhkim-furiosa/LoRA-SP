...
class LoraMSPLinear(LoraLinear):
    ...
    def _fill_cache(
        ...
        """Store tensors safely for DDP/checkpointing: keep only detached tensors separated from the graph."""
        if detach:
    ...
    def compute_balance_loss(self) -> torch.Tensor:
        ...
        # p_j: mean gate probability per expert
        p = self._last_gates.mean(dim=tuple(range(self._last_gates.dim() - 1)))  # (E,)

        # f_j: actual token dispatch fraction (hard one-hot)
    ...
    def _topk_vals(
        ...
        for r in [ground_rank, target_rank]:
            ...
            max_k = r.max().item()  # largest k across the batch
